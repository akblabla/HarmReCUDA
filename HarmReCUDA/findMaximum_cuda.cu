...
void findMaximum_kernel(matrix a_d)
{
	...
	if (columnIndex < a_d.columns) {//make sure not to write outside of matrix, incase the number of elements did not have a base of 1024
		...
		for (int rowIndex = 0; rowIndex < a_d.rows; ++rowIndex) {
			if (a_d.elements[MATRIX_INDEX(rowIndex, columnIndex, a_d.ld)]>largestValue) {
				a_d.elements[MATRIX_INDEX(largestIndex, columnIndex, a_d.ld)] = 0;
				largestIndex = rowIndex;
				largestValue = a_d.elements[MATRIX_INDEX(rowIndex, columnIndex, a_d.ld)];
				a_d.elements[MATRIX_INDEX(largestIndex, columnIndex, a_d.ld)] = 1;
			}
			else
			{
				a_d.elements[MATRIX_INDEX(rowIndex, columnIndex, a_d.ld)] = 0;
			}
		}
	}
}
