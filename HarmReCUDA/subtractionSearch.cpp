...
void subtractionSearch_d(Matrix_d& destination, const Matrix_d& src, int harmonicCount)
{
	if (src.getRows() % (2 * harmonicCount) != 0) {
		throw std::exception("The amount of rows in the harmonic energy matrix must be a factor of the amount of harmonics times 2");
		return;
	}
